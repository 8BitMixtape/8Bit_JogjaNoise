...
void loop() {
  ...
  LedPosition   &= 0b00000011; // prevent out of range
  ...
  if (gateOpen) {
    ...
    switch (type) {
      ...
      case 0:                                         // Square
        ...
        int temp, temp2;
        OCR1A = ((((Acc >> 8) & 0x80) + ((Acc2 >> 8) & 0x80)) * vol) >> 8;
        break;

      case 1:                                         // Ramp Up

        OCR1A = (((Acc >> 9) + (Acc2 >> 9)) * vol) >> 8;
        break;
      ...
      case 2:                                         // Triangle
        ...
        Temp2 = Acc2 >> 8;
        Mask2 = Temp2 >> 7;
        OCR1A = (((Temp ^ Mask) + (Temp2 ^ Mask2)) * vol) >> 8 ;
        break;

      case 3:                                         // Sinus

        OCR1A = (((pgm_read_byte(&sine256[Acc >> 8]) + pgm_read_byte(&sine256[Acc2 >> 8])) >> 1) * vol) >> 8;
        break;
